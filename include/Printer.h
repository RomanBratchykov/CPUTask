...
public:
    ...
    static void printToFile(std::filesystem::path const& path, std::vector<CpuCore> & cpu_cores, std::string const& time);

    static void printToFile(std::filesystem::path const& path, std::vector<CpuCore> & cpu_cores, std::string const& time, int core);
